...
## Portable Single-EXE Windows Build

Darkstar includes a local Windows builder that packages the current `backend/Darkstar_Core.js` and `backend/Darkstar_Renderer.js` monoliths together with Electron and Darkstar-managed native runtimes into one no-install Windows x64 executable.

Run on the Windows PC that will perform the build:
...
```powershell
powershell.exe -NoProfile -File backend\scripts\build-portable-single-exe.ps1
```

Build prerequisites are Node.js 20+ with npm and 64-bit Python 3.11. The builder downloads/stages the Electron version pinned by `backend/runtime-component-policy.json`, the pinned llama.cpp CPU/Vulkan/CUDA runtimes, the pinned stable-diffusion.cpp CPU/Vulkan runtimes, a private Python runtime with the bundled PDF/image requirements, and a standalone Node runtime. By default Tesseract OCR must also be installed on the build PC so its runtime and English language data can be embedded; `-SkipTesseract` is available only when intentionally building without OCR.

The result is written to:

```text
portable-release\Darkstar-Portable.exe
```

`portable-release\` contains only that executable. Models are user data and are intentionally not embedded. When the portable executable runs from a writable directory, managed projects, models, and Darkstar user data persist beside the EXE instead of inside Electron's temporary extraction directory.



## Portable single-EXE builder (local Windows build)

Run `Build_Portable_EXE.bat` on the Windows build PC. The builder reuses the existing Darkstar `venv`, its Python 3.11 base runtime, and the already-provisioned Electron/llama.cpp/stable-diffusion.cpp runtimes. It does not recreate the venv or run `pip install`. Tesseract remains optional unless `-RequireTesseract` is explicitly supplied. Runtime chat metadata (`projects/**/.darkstar`) and Python cache files are excluded only from isolated verification/packaging mirrors and are never deleted from the live project tree.


### Portable EXE builder notes

`Build_Portable_EXE.bat` reuses Darkstar's existing `venv` and already-provisioned runtimes. It does not recreate the venv or run `pip install`. On the first build only, the script installs `electron-builder` and its transitive **build-time** packages under `.darkstar-build/portable-builder-tools`; that cache is excluded from the staged application and is not bundled as Darkstar runtime dependencies. Generated `package.json`, launcher JavaScript, and build metadata are written as strict UTF-8 **without a BOM** so Windows PowerShell 5.1 cannot produce JSON that `electron-builder` rejects.
