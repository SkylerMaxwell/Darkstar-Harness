...
Darkstar Harness first-party source is licensed solely under Apache-2.0. The root `LICENSE` is the only first-party licensing document; do not reintroduce Community/Teams/Enterprise license files, a CLA, or another first-party licensing regime. Third-party components must retain their required upstream notices.

The root `Launch_Darkstar.bat` is the supported Windows source-checkout entrypoint. It must remain location-independent, preserve argument forwarding, bootstrap only the pinned Electron and llama.cpp runtimes when required, and launch `backend/Darkstar_Core.js` with runtime diagnostics enabled. Runtime provisioning is the only normal case in which a console may remain visible: when Electron or llama.cpp assets are missing, setup must be visibly relaunched through `backend/scripts/launch-darkstar-setup.vbs`, show download/provisioning progress, and close automatically after setup hands the application to `backend/scripts/launch-darkstar-hidden.vbs`. Once runtimes are ready, normal launches must hand off to the hidden helper. Darkstar must not create, copy, register, or maintain Windows shortcuts in the project directory, Desktop, or Start Menu. The obsolete root native `Darkstar.exe`, its Go launcher sources, and its rebuild workflow must remain absent. Hidden BAT startup failures must return the failing exit code to the helper so the helper can report the error without an invisible pause.
