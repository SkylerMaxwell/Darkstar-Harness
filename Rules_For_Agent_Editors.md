...
Project chat persistence is project-owned. Durable chat content belongs under the project's reserved `.darkstar/` metadata directory, not in a renderer-origin preference store or one global application session blob. Core owns the authoritative project root used for persistence; renderer-supplied workspace paths must never redirect writes. The reserved metadata directory must stay hidden from ordinary Workspace File Explorer operations and inaccessible to scoped filesystem tools. External project roots may be remembered globally only as lightweight pointers needed to rediscover their project-owned session files.
