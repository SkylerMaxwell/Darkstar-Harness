...
3. If any generated backend directory contains an additional `.exe` or `.dll` not already classified by policy, review its redistribution terms and add an explicit `manualDeclarations` entry to `backend/runtime-component-policy.json`, including every required notice file and `"redistributionReviewed": true`.
